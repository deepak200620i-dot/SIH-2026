"""
IBVAP — Tracking + Virtual Fence Demo Script
================================================
Opens a video file, runs YOLO26n tracking with ByteTrack per frame,
displays annotated output with persistent track IDs and virtual fence
zone overlays.  Prints intrusion alerts to the console.

Usage:
    python scripts/run_tracking_demo.py
    python scripts/run_tracking_demo.py --video path/to/video.mp4
    python scripts/run_tracking_demo.py --save   # saves output video instead of displaying
"""
...
def run_demo(
    ...
    """Run the tracking + virtual fence demo on a video file."""
    ...
    for z in fence.zones:
        print(f"       • {z.name} [{z.severity.upper()}] — {len(z.polygon)} vertices")
    ...
    if save_output:
        out_path = output_path or str(
            PROJECT_ROOT / "data" / "evidence" / "tracking_demo_output.mp4"
        )
    ...
    try:
        while True:
            ...
            # Annotate: draw fence zones first, then tracked objects on top
            ...
            # Intrusion count on HUD
            if total_intrusion_events > 0:
                alert_hud = f"Intrusion Alerts: {total_intrusion_events}"
                cv2.putText(
                    annotated, alert_hud, (10, 60),
            ...
            if frame_idx % 30 == 0:
                track_summary = ", ".join(
                    f"ID:{o.track_id}:{o.class_name}"
                    for o in result.tracked_objects[:5]
                )
                if len(result.tracked_objects) > 5:
                    track_summary += f" ... +{len(result.tracked_objects) - 5} more"
                print(f"  {hud}  [{track_summary}]")

            # Save or display
            if writer:
                writer.write(annotated)
            else:
                cv2.imshow("IBVAP Tracking + Fence Demo", annotated)
    ...
    print(f"  Fence zones active : {len(fence.zones)}")
    print(f"{'='*60}")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="IBVAP Tracking + Virtual Fence Demo",
    )
    parser.add_argument(
