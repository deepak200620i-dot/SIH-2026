...
// ============ FACE EVENTS ============
export const generateMockFaceEvent = (cameraId, index) => ({
    id: generateId(),
    cameraId,
...
export const mockFaceEvents = Array.from({ length: 8 }, (_, i) => generateMockFaceEvent(mockCameras[i % 4].id, i));
// ============ ANPR EVENTS ============
export const generateMockANPREvent = (cameraId, index) => ({
    id: generateId(),
    cameraId,
    timestamp: generateTimestamp(Math.floor(Math.random() * 60)),
...
export const mockANPREvents = Array.from({ length: 6 }, (_, i) => generateMockANPREvent(mockCameras[i % 4].id, i));
// ============ SECURITY EVENTS ============
export const generateMockSecurityEvent = (cameraId, eventType) => {
...
export const mockSecurityEvents = Array.from({ length: 25 }, (_, i) => generateMockSecurityEvent(mockCameras[i % 4].id, eventTypes[i % eventTypes.length])).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
// ============ ALERTS ============
export const mockAlerts = mockSecurityEvents
