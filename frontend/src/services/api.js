...
export async function getStats() {
  try {
    const response = await apiClient.get('/api/events/stats');
    return response.data;
  } catch (err) {
    const fallback = await apiClient.get('/api/stats');
...
export function getEvidenceUrl(filename) {
  ...
  const cleanName = filename.replace(/^data\/evidence\//, '');
  return `${API_BASE}/api/evidence/${cleanName}`;
}
