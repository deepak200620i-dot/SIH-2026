...
export const apiGetCameraStatus = async (cameraId: string): Promise<any> => {
  try {
    ...
    if (res.ok) {
      ...
      return {
        cameraId,
        peopleCount: items.filter((e: any) => e.class_name === "person").length || 1,
        vehicleCount: items.filter((e: any) => e.class_name === "car").length || 0,
    ...
    if (res.ok) {
      ...
      if (zones.length > 0) {
        return zones.map((z: any, idx: number) => ({
          ...
          status: "ACTIVE",
          polygon: (z.polygon || []).map(([x, y]: [number, number]) => ({ x, y })),
        }));
      }
    }
  } catch (err) {}
