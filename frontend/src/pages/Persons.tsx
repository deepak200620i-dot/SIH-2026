...
export const Persons: React.FC = () => {
  ...
  return <div className="p-6 space-y-6"><div><h1 className="text-2xl font-bold">Known Persons</h1><p className="text-sm text-gray-500">Face-gallery records used by the recognition engine.</p></div>
    ...
    {people.length === 0 ? <div className="panel p-10 text-center text-gray-500"><Users className="mx-auto mb-3 text-maroon-700"/>No known persons enrolled.</div> : <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-4">{people.map(person => <div key={person.id} className="panel overflow-hidden"><div className="aspect-square bg-maroon-50 flex items-center justify-center">{person.photoUrl ? <img src={person.photoUrl} alt={person.name} className="h-full w-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = "/favicon.svg"; }} /> : <Users className="text-gray-400 size-8"/>}</div><div className="p-3 flex gap-2 items-center"><p className="font-medium text-sm truncate flex-1">{person.name}</p><button onClick={() => remove(person.id)} className="text-red-700 hover:text-red-900" aria-label={`Remove ${person.name}`}><Trash2 size={16}/></button></div></div>)}</div>}</div>;
};
