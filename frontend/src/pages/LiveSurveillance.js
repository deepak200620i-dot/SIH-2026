...
import { AlertCircle, Users, Truck, Maximize2, Video, Upload, Plus, X, Play, Square, CheckCircle2, Cpu, Layers, AlertTriangle, Trash2, } from "lucide-react";
import { apiUploadVideo, apiProcessWebcamFrame, apiStopCamera, apiAddCamera, apiDeleteCamera, } from "@/services/api";
const CameraPreview = ({ cameraId, name, source }) => {
    ...
    if (error && usingDirectStream)
        return _jsxs("div", { className: "text-center p-6 space-y-2", children: [_jsx("div", { className: "text-4xl", children: "\uD83D\uDCF9" }), _jsx("p", { className: "text-amber-300 text-sm font-medium", children: "Feed unavailable" }), _jsxs("p", { className: "text-gray-500 text-xs", children: ["Use the phone app's ", _jsx("code", { children: "/video" }), " or RTSP stream URL\u2014not ", _jsx("code", { children: "greet.html" }), "."] })] });
    return _jsx("img", { src: usingDirectStream ? directSource : src, onLoad: () => setError(false), onError: () => { if (!usingDirectStream && directSource?.startsWith("http"))
            setUsingDirectStream(true);
        else
            setError(true); }, className: "absolute inset-0 h-full w-full object-cover", alt: `${name} live feed` });
};
export const LiveSurveillance = () => {
            ...
            if (res && canvasRef.current) {
                ...
                if (ctx) {
                    ...
                    res.tracked_objects.forEach((obj) => {
                        ...
                        let label = `${obj.class_name.toUpperCase()} #${obj.track_id} (${Math.round(obj.confidence * 100)}%)`;
                        if (obj.face_name) {
                            if (obj.is_known_face) {
                                boxColor = "#22c55e"; // green for known face
                                tagColor = "#15803d";
                                label = `PERSON: ${obj.face_name.replace(/_/g, " ")} (${Math.round(obj.confidence * 100)}%)`;
                            }
                            else {
                                boxColor = "#f97316"; // orange for unknown face
                                tagColor = "#c2410c";
                                label = `UNKNOWN PERSON #${obj.track_id} (${Math.round(obj.confidence * 100)}%)`;
                            }
                        }
    ...
    const handleDeleteCamera = async (camera) => {
        if (!window.confirm(`Delete ${camera.name}? This removes only the saved camera source.`))
            return;
