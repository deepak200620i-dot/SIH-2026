...
const CameraPreview = ({ cameraId, name }) => {
    ...
    if (error)
        return _jsxs("div", { className: "text-center p-6 space-y-2", children: [_jsx("div", { className: "text-4xl", children: "\uD83D\uDCF9" }), _jsx("p", { className: "text-amber-300 text-sm font-medium", children: "Feed unavailable" }), _jsx("p", { className: "text-gray-500 text-xs", children: "Check the configured camera source and connection." })] });
    return _jsx("img", { src: src, onLoad: () => setError(false), onError: () => setError(true), className: "absolute inset-0 h-full w-full object-cover", alt: `${name} live feed` });
};
